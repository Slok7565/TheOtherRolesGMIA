...
namespace TheOtherRoles
{
    ...
    class GameOptionsMenuStartPatch {
        ...
        private static void createClassicTabs(GameOptionsMenu __instance) {
            bool isReturn = setNames(
                new Dictionary<string, string>() {
                    ["TORSettings"] = ModTranslation.getString("theOtherRolesSettings"),
                    ["ImpostorSettings"] = ModTranslation.getString("impostorRolesSettings"),
                    ["NeutralSettings"] = ModTranslation.getString("neutralRolesSettings"),
                    ["CrewmateSettings"] = ModTranslation.getString("crewmateRolesSettings"),
                    ["ModifierSettings"] = ModTranslation.getString("modifierSettings")
                });
        ...
        private static void createGuesserTabs(GameOptionsMenu __instance) {
            bool isReturn = setNames(
                new Dictionary<string, string>() {
                    ["TORSettings"] = ModTranslation.getString("theOtherRolesSettings"),
                    ["GuesserSettings"] = ModTranslation.getString("guesserSettings"),
                    ["ImpostorSettings"] = ModTranslation.getString("impostorRolesSettings"),
                    ["NeutralSettings"] = ModTranslation.getString("neutralRolesSettings"),
                    ["CrewmateSettings"] = ModTranslation.getString("crewmateRolesSettings"),
                    ["ModifierSettings"] = ModTranslation.getString("modifierSettings")
        ...
        private static void createHideNSeekTabs(GameOptionsMenu __instance) {
            bool isReturn = setNames(
                new Dictionary<string, string>(){ 
                    ["TORSettings"] = ModTranslation.getString("theOtherRolesSettings"), 
                    ["HideNSeekSettings"] = ModTranslation.getString("hideNSeekSettings") 
                });
    ...
    class GameOptionsDataPatch
    {
        ...
        private static string buildOptionsOfType(CustomOption.CustomOptionType type, bool headerOnly) {
            ...
            foreach (var option in options) {
                if (option.parent == null) {
                    ...
                    if (option.id == 103) //Deputy
                        sb.AppendLine($"- {OtherHelper.cs(Deputy.color, ModTranslation.getString("deputy"))}: {option.getString()}");
                    else if (option.id == 224) //Sidekick
                        sb.AppendLine($"- {OtherHelper.cs(Sidekick.color, ModTranslation.getString("sidekick"))}: {option.getString()}");
                    else if (option.id == 8000) // Created Madmate
                        sb.AppendLine($"- {OtherHelper.cs(Madmate.color, Madmate.fullName)}: {option.getString()}");
                    //else if (option.id == 358) //Prosecutor
                    //sb.AppendLine($"- {OtherHelper.cs(Lawyer.color, "Prosecutor")}: {option.selections[option.selection].ToString()}");
                }
            }
            ...
            foreach (CustomOption option in options) {
                ...
                if (option.parent != null) {
                    ...
                    sb.AppendLine(OtherHelper.cs(c, $"{option.getName()}: {option.getString()}"));
                } else {
        ...
        public static string buildAllOptions(string vanillaSettings = "", bool hideExtras = false) {
            ...
            string hudString = counter != 0 && !hideExtras ? OtherHelper.cs(DateTime.Now.Second % 2 == 0 ? Color.white : Color.red, $"{ModTranslation.getString("useScrollWheel")}\n\n") : "";
            int maxPage = 7;
    ...
    public class HudManagerUpdate {
        ...
        public static void Prefix2(HudManager __instance) {
            ...
            for (int i = 0; i < blocks.Length; i++) {
                ...
                curBlock = blocks[i];
                if (Helpers.lineCount(curBlock) + Helpers.lineCount(curString) < (AmongUs.Data.DataManager.Settings.Language.CurrentLanguage == SupportedLangs.English ? 43 : 40)) { // original: 43
                    curString += curBlock + "\n\n";
                } else {
